...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function initialize(bytes32 _tokenHash, address _dispenser, address _stonkToken) external {
    ...
    function burn(address to) external lock returns (uint tokenAmount, uint stonkAmount) {
        ...
        _safeStonkTransfer(stonkToken, to, stonkAmount);
    ...
    function swap(uint tokenAmountOut, uint stonkAmountOut, address to, bytes calldata data) external lock {
        ...
        require(to != stonkToken, 'UniswapV2: INVALID_TO');
        // TODO: rewrite transfer
        if (tokenAmountOut > 0) _safeTokenTransfer(_tokenHash, to, tokenAmountOut); // optimistically transfer tokens
        if (stonkAmountOut > 0) _safeStonkTransfer(stonkToken, to, stonkAmountOut); // optimistically transfer tokens
